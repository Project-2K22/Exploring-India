...
const modelstyle = {
    //EDIT INFORMATION MODEL BOX STYLE
    position: 'absolute',
    top: '50%',
    left: '50%',
...
const deletemyaccountmodelstyle = {
    // DELETE YOUR ACCOUNT CON MODEL BOX STYLE
    position: 'absolute',
    top: '50%',
    left: '50%',
    transform: 'translate(-50%, -50%)',
...
const UserProfile = () => {
    ...
    const uploadProfileImg = () => {
        ...
        uploadTask.on('state_changed', usnapshot => {
            setAlert({
                visible: true,
                message: 'Uploading ' + Math.round((usnapshot.bytesTransferred / usnapshot.totalBytes) * 100),
                type: 'success',
            });
        }); //UPLOADING PROGRESS
    ...
    const handelUpdateChanges = () => {
        //UPDATING DATA MAIN FUNCTION

        handleClosefe();
    };

    const editFieldsChange = e => {
        //UPDATING DATA MAIN FUNCTION
        ...
        if (nameName === 'name') {
            update(ref(db, 'users/' + uid), {
                ...
                .then(function () {
                    setAlert({ visible: true, message: 'Name Updated', type: 'success' });
                })
                .catch(function (error) {
                    const errorMessage = error.message;
                    setAlert({ visible: true, message: errorMessage, type: 'error' });
        ...
        if (nameName === 'phoneno') {
            update(ref(db, 'users/' + uid), {
                ...
                .then(function () {
                    setAlert({ visible: true, message: 'Phoneno Updated', type: 'success' });
                })
                .catch(function (error) {
                    const errorMessage = error.message;
                    setAlert({ visible: true, message: errorMessage, type: 'error' });
        ...
        if (nameName === 'age') {
            update(ref(db, 'users/' + uid), {
                ...
                .then(function () {
                    setAlert({ visible: true, message: 'Age Updated', type: 'success' });
                })
                .catch(function (error) {
                    const errorMessage = error.message;
                    setAlert({ visible: true, message: errorMessage, type: 'error' });
        ...
        if (nameName === 'gender') {
            update(ref(db, 'users/' + uid), {
                ...
                .then(function () {
                    setAlert({ visible: true, message: 'Gender Updated', type: 'success' });
                })
                .catch(function (error) {
                    const errorMessage = error.message;
                    setAlert({ visible: true, message: errorMessage, type: 'error' });
    ...
    const deleteTheUser = () => {
        ...
        if (password === '') {
            ...
            reauthenticateWithCredential(duser, credential)
                .then(() => {
    ...
    const handlePreferenecChange = e => {
        if (e.target.id === 'whattype') {
            update(ref(db, 'users/' + uid), {
                ...
                .then(function () {
                    setAlert({ visible: true, message: 'Data Updated', type: 'success' });
                })
                .catch(function (error) {
                    const errorMessage = error.message;
                    setAlert({ visible: true, message: errorMessage, type: 'error' });
                    ...
                    >
                        ...
                        <Box
                            sx={{
                                ...
                                width: '50%',
                                height: '100%',
                                background: 'linear-gradient(to right, #ea5b6c, #ec6667, #ec7263, #ec7d61, #eb8860)',
                            ...
                            <Stack spacing={0} justifyContent="center" alignItems="center" height="100%">
                                ...
                                <Box sx={{ width: '100%', height: '55%' }}>
                                    <Stack spacing={6} alignItems="center" height="100%">
                                        ...
                                        <Button variant="contained" color="info" onClick={handleOpenfe}>
                                            EDIT
                                        </Button>{' '}
                                        {/*OPEN THE EDIT MODEL BUTTON*/}
                                        {/* <Button
                                            variant="contained"
                                            color="success"
                                            startIcon={<HomeIcon />}
                            ...
                            <Stack p={2} pb={0}>
                                ...
                                >
                                    <Box>
                                        ...
                                        >
                                            <h3>Email</h3>
                                            <Typography variant="h3" style={informationstyle}>
                                                {uf_user_data.email}
                                            </Typography>
                                        </Stack>
                                    </Box>
                                    <Box>
                                        ...
                                        >
                                            <h3 p={0}>Phone No</h3>
                                            <Typography variant="h3" style={informationstyle}>
                                                {uf_user_data.phoneno === -1 ? '**********' : uf_user_data.phoneno}
                                            </Typography>
                                        </Stack>
                                    </Box>
                                </Stack>
                                ...
                                >
                                    <Box>
                                        ...
                                        >
                                            <h3>Age</h3>
                                            <Typography variant="h3" style={informationstyle}>
                                                {uf_user_data.age === -1 ? '***' : uf_user_data.age}
                                            </Typography>
                                        </Stack>
                                    </Box>
                                    <Box>
                                        ...
                                        >
                                            <h3>Gender</h3>
                                            <Typography variant="h3" style={informationstyle}>
                                                {uf_user_data.gender === -1 ? '****' : uf_user_data.gender}
                                            </Typography>
                                        </Stack>
                                    </Box>
                                </Stack>
                                                    ...
                                                    {whattypeArr.map(val => {
                                                        ...
                                                        if (val.toLowerCase() !== user_lengthOfTrip.toLowerCase()) {
                                                            ...
                                                            return (
                                                                <option
                                                                    selected
                                                                    value={user_lengthOfTrip}
                                                                    key={user_lengthOfTrip}
                                                                >
                                                                    {user_lengthOfTrip.toUpperCase()}
                                                                </option>
                                                            );
                                                        }
                                                    })}
                                            ...
                                            <Box>
                                                ...
                                                >
                                                    {lengthOfTripArr.map(val => {
                                                        ...
                                                        if (myowncounterstart === 0) {
                                                            myowncounterend = lengthOfTripArr.length;
                                                        }
                                                        myowncounterstart = myowncounterstart + 1;
                                            ...
                                            <Box>
                                                ...
                                                >
                                                    {outsidestateArr.map(val => {
                                                        const fromArrVal = val;
                                                        const user_lengthOfTrip =
                                                            uf_user_data.wantToVisitPlacesOtherStates;
                                                        if (myowncounterstart === 0) {
                                                            myowncounterend = outsidestateArr.length;
                                                        }
                                                        myowncounterstart = myowncounterstart + 1;
                                            ...
                                            <Box>
                                                ...
                                                >
                                                    {whatweatherArr.map(val => {
                                                        ...
                                                        if (myowncounterstart === 0) {
                                                            myowncounterend = whatweatherArr.length;
                                                        }
                                                        myowncounterstart = myowncounterstart + 1;
                    ...
                    >
                        <Box sx={modelstyle}>
                            ...
                            <Stack
                                ...
                                alignItems="stretch"
                            >
                                <Alert severity="info">{allmessages}</Alert>
                                <FormControl fullWidth variant="filled">
                                    <InputLabel htmlFor="outlined-adorment-name">Name</InputLabel>
                                    <FilledInput
                                        name="name"
                                        id="outlined-adorment-name"
                                        type="text"
                                        defaultValue={uf_user_data.name === '' ? '***' : uf_user_data.name}
                                        onChange={editFieldsChange}
                                    />
                                </FormControl>
                                <FormControl fullWidth variant="filled">
                                    <InputLabel htmlFor="outlined-adorment-phoneno">Phone Number</InputLabel>
                                    <FilledInput
                                        name="Phone_number"
                                        id="outlined-adorment-phoneno"
                                        type="tel"
                                        defaultValue={uf_user_data.phoneno === -1 ? '***' : uf_user_data.phoneno}
                                        onChange={editFieldsChange}
                                    />
                                </FormControl>
                                <FormControl fullWidth variant="filled">
                                    <InputLabel htmlFor="outlined-adorment-age">Age</InputLabel>
                                    <FilledInput
                                        name="age"
                                        id="outlined-adorment-age"
                                        type="number"
                                        defaultValue={uf_user_data.age === -1 ? '***' : uf_user_data.age}
                                        onChange={editFieldsChange}
                                    />
                                </FormControl>
                                <FormControl fullWidth variant="filled" sx={{ m: 1, minWidth: 215 }}>
                                    ...
                                    <Select
                                        ...
                                        defaultValue={uf_user_data.gender === '' ? '***' : uf_user_data.gender}
                                        onChange={editFieldsChange}
                                    >
